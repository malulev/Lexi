...
| A5 | RAM vs summed ceiling | `node_memory_MemAvailable_bytes{project="lexi"} < lexi_agents_limit_total * 419430400` | 15m | The question nothing in the product can express. `for: 15m` because `slots.ts` is a check-then-act race and a momentary overshoot is expected. |
